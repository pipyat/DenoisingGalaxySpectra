...
# Skip connection
skip = layers.Concatenate(name="concat_skip")([x_b1, x_r1])
...
skip = layers.Concatenate(name="concat_skip2")([x, skip])
...
narrow_windows_skip = Model(inputs=[input_b, input_r], outputs=output, name="TwoHeadedSpectraModel")
...
narrow_windows_skip.compile(optimizer=optimizer, loss=loss)

print(narrow_windows_skip.summary())

# Fit the model with callbacks
narrow_windows_skip.fit(
    x=[b_train, r_train],
    y=flux_train,
